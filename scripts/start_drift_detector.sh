docker run --rm --name $DRIFT_DETECTOR_HOST \
  ...
  -e RAFIKI_IMAGE_DRIFT_DETECTOR_WORKER=$RAFIKI_IMAGE_DRIFT_DETECTOR_WORKER \
  -e RAFIKI_VERSION=$RAFIKI_VERSION \
  -v /var/run/docker.sock:/var/run/docker.sock \
  -v $LOGS_FOLDER_PATH:$LOGS_FOLDER_PATH \
  -p 8005:$DRIFT_DETECTOR_PORT \
  $RAFIKI_IMAGE_DRIFT_DETECTOR:$RAFIKI_VERSION