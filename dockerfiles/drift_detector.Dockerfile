...
COPY rafiki/drift_detector/requirements.txt drift_detector/requirements.txt
RUN pip install -r drift_detector/requirements.txt

COPY rafiki/ rafiki/

# Copy init script
COPY scripts/start_drift_detector.py start_drift_detector.py
